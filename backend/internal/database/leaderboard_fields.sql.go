...
const createLeaderboardField = `-- name: CreateLeaderboardField :one
INSERT INTO leaderboard_fields (
        lid,
        field_name,
        field_value,
...
type CreateLeaderboardFieldParams struct {
	Lid        int32
	FieldName  string
...
func (q *Queries) CreateLeaderboardField(ctx context.Context, arg CreateLeaderboardFieldParams) (int32, error) {
	row := q.db.QueryRow(ctx, createLeaderboardField,
		arg.Lid,
		arg.FieldName,
		arg.FieldValue,
		arg.FieldOrder,
...
type CreateLeaderboardFieldsParams struct {
	Lid        int32
	FieldName  string
	FieldValue FieldType
	FieldOrder int32
...
SELECT lid, field_name, field_value, field_order, for_rank, hidden, required, id, default_value
