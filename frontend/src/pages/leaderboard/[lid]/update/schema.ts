...
const LbMetadataSchema = y.object({
  id: y.number().required("Id is required"),
  ...
  externalLinks: y
    .array(
      y.object().shape({
        id: y.number().required("external link's id is required"),
        displayValue: y
          .string()
...
export const FieldsSchema = y.object().shape({
  fields: y
    .array(
      y.object().shape({
        name: y
...
export type EditFieldType = y.InferType<typeof FieldsSchema>;
export type EditMetadataType = y.InferType<typeof LbMetadataSchema>;
